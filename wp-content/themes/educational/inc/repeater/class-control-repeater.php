...
/*
  * This class of code is derived from Blossom Coach,  Copyright 2017  blossomthemes.com Blossom Coach is distributed under the terms of the GNU GPL
  * Author: Blossom Themess
*/
...
if( ! class_exists( 'Educational_Control_Repeater' ) ) {
    /**
     * Repeater control
     */
    class Educational_Control_Repeater extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            ...
            $this->row_label = array(
                'type' => 'text',
                'value' => esc_attr__( 'row', 'educational' ),
                'field' => false,
            );
            ...
            if ( empty( $this->button_label ) ) {
             ...
             $this->button_label = sprintf( esc_attr__( 'Add new %s', 'educational' ), $this->row_label['value'] );
         }
        ...
        public function enqueue() {    
            ...
            wp_enqueue_script( 'educational-repeater', get_template_directory_uri() . '/assets/customizer/repeater.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
            wp_enqueue_style( 'educational-repeater', get_template_directory_uri() . '/assets/customizer/repeater.css', null );
        }
        ...
        protected function render_content() {
            ...
            <?php if ( isset( $this->choices['limit'] ) ) : ?>
                ...
                printf( esc_attr__( 'Limit: %s rows', 'educational' ), esc_html( $this->choices['limit'] ) ); ?></p>
            <?php endif; ?>
        ...
        public function repeater_js_template() {
            ...
            <label>
                ...
                <span class="description customize-control-description">{{{ field.description }}}</span>
                <# } #>
                <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'educational' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
            ...
            <figure class="kirki-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'educational' ); ?>" >
                <# if ( field.default ) { #>
                <# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
                <img src="{{{ defaultImageURL }}}">
                <# } else { #>
                <?php esc_html_e( 'No Image Selected', 'educational' ); ?>
                <# } #>
            </figure>
            
            <div class="actions">
                <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'educational' ); ?></button>
                <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add Image', 'educational' ); ?>" data-alt-label="<?php esc_attr_e( 'Change Image', 'educational' ); ?>" >
                    <# if ( field.default ) { #>
                    <?php esc_html_e( 'Change Image', 'educational' ); ?>
                    <# } else { #>
                    <?php esc_html_e( 'Add Image', 'educational' ); ?>
                    <# } #>
                </button>
                <# if ( field.default.id ) { #>
            ...
            <figure class="kirki-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'educational' ); ?>" >
                <# if ( field.default ) { #>
                ...
                <# } else { #>
                <?php esc_html_e( 'No File Selected', 'educational' ); ?>
                <# } #>
            </figure>
            
            <div class="actions">
                <button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
                <button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'educational' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'educational' ); ?>" >
                    <# if ( field.default ) { #>
                    <?php esc_html_e( 'Change File', 'educational' ); ?>
                    <# } else { #>
                    <?php esc_html_e( 'Add File', 'educational' ); ?>
                    <# } #>
                </button>
                <# if ( field.default.id ) { #>
                <input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
        ...
        <# }); #>
        <button type="button" class="button-link repeater-row-remove"><?php esc_html_e( 'Remove', 'educational' ); ?></button>
    </div>
</li>
</script>
<?php
